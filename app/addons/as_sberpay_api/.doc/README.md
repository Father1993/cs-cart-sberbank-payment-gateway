...
После успешного `register.do` (когда в запрос уходит фискальный `orderBundle`) модуль сохраняет в таблице **`?:sberpay_order_meta`** снимок корзины, ушедший в шлюз: **`fiscal_snapshot`** (исходный `order_bundle`, денормализованные `items`, суммы в копейках, `gateway_order_id`, `order_number` и т.д.). Это **канонический** источник для чека возврата при **полном** возврате остатка. Статус **«Чек предоплаты»** в админке берётся из **`prepayment_receipt`** (OFD `getReceiptStatus`, первый `sell`), а не из наличия `fiscal_snapshot` alone; без оплаты отображается **«Не отправлен»**. Закрывающий — **`closing_receipt`** (второй и далее `sell`). Чек возврата — **`refund_receipt`** (`sell_refund` в OFD); кнопка «Запросить статус из OFD» и события оплаты/возврата/C вызывают один `getReceiptStatus` и обновляют все три meta.

В блоках **`prepayment_receipt`**, **`closing_receipt`**, **`refund_receipt`** дополнительно сохраняются опциональные **`receipt_url`** (ссылка на чек в ОФД) и **`receipt_id`** (`receiptId` / `externalId` из `receipts[]`). URL извлекается из **`receipts[].payload.ofdReceiptUrl`** (OFD v1 на проде); при пустом ответе OFD поле не перезаписывается — merge-on-save сохраняет уже записанный URL. Сразу после оплаты (`payment_confirmed`) URL часто ещё отсутствует (`receiptStatus=1`); ссылка появляется при `receiptStatus=3` после повторного poll (кнопка в админке, `doReceipt_success`, смена статуса на «Выполнен»).
