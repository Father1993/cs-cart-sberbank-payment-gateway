...
После успешного `register.do` (когда в запрос уходит фискальный `orderBundle`) модуль сохраняет в таблице **`?:sberpay_order_meta`** снимок корзины, ушедший в шлюз: **`fiscal_snapshot`** (исходный `order_bundle`, денормализованные `items`, суммы в копейках, `gateway_order_id`, `order_number` и т.д.). Это **канонический** источник для чека возврата при **полном** возврате остатка. Статус **«Чек предоплаты»** в админке берётся из **`prepayment_receipt`** (OFD `getReceiptStatus`, первый `sell`), а не из наличия `fiscal_snapshot` alone; без оплаты отображается **«Не отправлен»**.
