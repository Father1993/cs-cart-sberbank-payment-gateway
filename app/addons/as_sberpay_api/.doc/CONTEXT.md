...
### 2. `receiptType: "income"` — значение не верифицировано
Невозможно проверить на тестовом аккаунте (нет АТОЛ).
Тестировать только на боевом `P272606974206`.
Возможные альтернативы: `"sell"`, `"INCOME"`, `1` (integer).

### 3. `ffdVersion: "1.05"` — формат предположительно верный
Убедиться что Сбер принимает строку `"1.05"`, а не `"FFD105"` или число.

### 4. ~~`orderBundle` без АТОЛ~~ → ПОДТВЕРЖДЕНО (итерация 4)
Тестовый аккаунт `sbertest_2221` **точно** не имеет АТОЛ-кассы.
**Любой** фискальный параметр вызывает errorCode: 5 на этом аккаунте.
Фискализацию тестировать ТОЛЬКО на боевом аккаунте с АТОЛ.
